...
import { toSql } from "pgvector/utils"; // <-- 1. IMPORT THE VECTOR HELPER
import { getEmbedding } from "@/lib/ai/embedding"; // <-- 2. IMPORT OUR EMBEDDING SERVICE
...
export async function POST(request: NextRequest, context: any) {
  try {
    ...
    const { rows } =
      await sql`SELECT id, name, avatar, prompt_persona FROM characters WHERE id = ${characterId};`;
    if (rows.length === 0)
      return NextResponse.json(
    ...
    try {
      const { rows: gpRows } =
        await sql`SELECT prompt_text FROM generic_prompts ORDER BY created_at DESC LIMIT 1;`;
      genericPrompt = gpRows[0]?.prompt_text ?? "";
    } catch (err) {
      console.warn(
        "Could not fetch generic prompt (it may not be initialized).",
        err,
      );
      genericPrompt = "";
    }
    ...
    const combinedPrompt = [genericPrompt, character.prompt_persona]
      .filter(Boolean)
      .join(" ");
    ...
    try {
      ...
      console.log({ embeddingSql, embedding });
      ...
      return NextResponse.json(
      {
        message: `Successfully generated a new joke for ${character.name}. and ✅ Memory saved successfully.`,
        character: character.name,
        avatar: character.avatar,
        joke: jokeContent,
      },
      ...
      console.error("⚠️ Failed to save memory for the new joke:", memoryError);
       return NextResponse.json(
      {
        message: `Successfully generated a new joke for ${character.name}. and ⚠️ Failed to save memory for the new joke:", memoryError`,
        character: character.name,
        avatar: character.avatar,
        joke: jokeContent,
      },
