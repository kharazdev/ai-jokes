...
// In our case, it will be { id: '...' } from the URL /characters/...
export default async function SingleJokePage({
  ...
  try {
    ...
    const { rows } = await sql<JokeCharacter>`
      SELECT * FROM characters WHERE id = ${jokeId};
    `;
    character = rows[0]; // Get the first (and only) result
  } catch (error) {
      ...
      <div className="w-full max-w-2xl">
        ...
        <div className="bg-white p-6 rounded-xl shadow-lg flex items-start space-x-4">
          ...
          <div>
            ...
            <p className="mt-4 text-xl text-gray-700">"{character?.bio}"</p>
